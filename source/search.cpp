...
int Search::AlphaBeta(int alpha, int beta, int depth, Board& b, SearchInfo& info, bool DoNull) {
  ...
  for (itr = list.m_move_vec.begin(); itr != list.m_move_vec.end(); ++itr) {
    ...
    */
    ...
    char str[50];
    if (depth == 1)
      sprintf(str, "%s node %ld, Score %d, alpha %d, beta %d\n", itr->moveString().c_str(), info.nodes, Score, alpha,
              beta);
    else if (depth == 2)
      sprintf(str, "	%s node %ld, Score %d, alpha %d, beta %d\n", itr->moveString().c_str(), info.nodes, Score,
              alpha, beta);
    else if (depth == 3)
      sprintf(str, "		%s node %ld, Score %d, alpha %d, beta %d\n", itr->moveString().c_str(), info.nodes,
              Score, alpha, beta);
    else
    ...
    if (Score > alpha) {
      ...
      if (!(itr->m_move & MFLAGCAP)) {
        assert(TOSQ(BestMove) < TOTAL_SQUARES);
...
void Search::SearchPosition(Board& b, SearchInfo& info) {
  ...
  for (int currentDepth = 1; currentDepth <= info.depth; ++currentDepth) {
    ...
    sprintf(str, "\nDepth:%d score:%d move:%s nodes:%ld ", currentDepth, bestScore, m.moveString().c_str(), info.nodes);
...
void Search::PickNextMove(std::vector<Move>::iterator move, MoveList& list) {
  std::vector<Move>::iterator result =
      std::max_element(move, list.m_move_vec.end(), [](Move a, Move b) { return a.m_score < b.m_score; });
