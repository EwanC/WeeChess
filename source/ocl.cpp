...
std::vector<Move> OCL::RunPawnMoveKernel(const Board& b, const bool capture)
{
    ...
    cl_mem Enpass_buffer = clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(unsigned int),
                                          (void*)&b.m_enPas, &err);
    if (err < 0)
    {
    ...
    /* Enqueue ReadBuffer */
    err = clEnqueueReadBuffer(m_queue, moves_buffer, CL_TRUE, 0, move_buffer_size * sizeof(unsigned long), moves, 0,
                              NULL, NULL);
    if (err < 0)
    {
...
std::vector<Move> OCL::RunPieceMoveKernel(const Board& b, const bool capture)
{
    ...
    cl_mem pieces_buffer = clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                          TOTAL_SQUARES * sizeof(int), (void*)b.m_board, &err);
    if (err < 0)
    {
        std::cout << "Couldn't create cl pawn peices buffer\n";
