...
#else
  std::string file(__FILE__);  // assume .cl files in same dir
...
void OCL::BuildProgram() {
  ...
  if (read_size != programSize) std::cout << "Reading Error\n";
...
std::vector<Move> OCL::RunPawnMoveKernel(const Board& b, const bool capture) {
  ...
  int err = CL_SUCCESS;
  cl_mem square_buffer = clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                        pawns * sizeof(unsigned int), (void*)pawn_squares, &err);
  if (err < 0) {
    std::cout << "Couldn't create cl pawn square buffer " << err << "\n";
    exit(1);
  }

  cl_mem pieces_buffer = clCreateBuffer(m_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                        TOTAL_SQUARES * sizeof(int), (void*)b.m_board, &err);
  if (err < 0) {
    std::cout << "Couldn't create cl pawn peices buffer\n";
    exit(1);
  }

  cl_mem Enpass_buffer =
      clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(unsigned int), (void*)&b.m_enPas, &err);
  if (err < 0) {
  ...
  cl_mem moves_buffer = clCreateBuffer(m_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                       move_buffer_size * sizeof(unsigned long), moves, &err);
  if (err < 0) {
  ...
  /* Enqueue ReadBuffer */
  err = clEnqueueReadBuffer(m_queue, moves_buffer, CL_TRUE, 0, move_buffer_size * sizeof(unsigned long), moves, 0, NULL,
                            NULL);
  if (err < 0) {
  ...
  for (int i = 0; i < move_buffer_size; ++i) {
    ...
    if (m.m_move & MFLAGEP) m.m_score = 105 + 1000000;
...
std::vector<Move> OCL::RunPieceMoveKernel(const Board& b, const bool capture) {
  ...
  int err;
  cl_mem square_buffer = clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                        global_size * sizeof(unsigned int), (void*)pieces, &err);
  if (err < 0) {
  ...
  cl_mem side_buffer =
      clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int), (void*)&b.m_side, &err);
  if (err < 0) {
    std::cout << "Couldn't create cl piece side buffer\n";
    exit(1);
  }

  cl_mem pieces_buffer = clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, TOTAL_SQUARES * sizeof(int),
                                        (void*)b.m_board, &err);
  if (err < 0) {
    std::cout << "Couldn't create cl pawn peices buffer\n";
    exit(1);
  ...
  unsigned long moves[move_buffer_size];
  for (int i = 0; i < move_buffer_size; ++i) moves[i] = 0;
...
int OCL::RunEvalKernel(const Board& board) {
  ...
  /* Create Buffers */
  int err;
  cl_mem bitboard_buffer = clCreateBuffer(m_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                          bitboards_per_board * sizeof(bitboard), (void*)board.m_pList, &err);
  if (err < 0) {
